...
class Model(object):
	...
	def __init__(self):
		...
		import time
		start = time.time()
		self.train_gen, self.test_gen = generate_cifar(self.batch_size, data_dir="cifar-10-batches-py")  #/shared/UsedDatabase/cifar-10-batches-py")
		print "Done. Took {:.2f} sec\n".format(time.time() - start)
		self.channels = 3
	...
	def build_deep_classifier_loss(self):
		with tf.variable_scope("capsnet_loss") as scope:
			...
			for i in range(self.num_classes):
				...
				self.deep_classifier_loss += loss
	...
	def check_test_accuracy(self, sess, test_set=None):
		batch_number = 0
		batch_accuracies = []
		...
		while batch_number < self.num_test_batches:
			...
			accuracy = sess.run(self.deep_classifier_accuracy, feed_dict={self.encoder_input: test_input,
																	 self.dropout: 1.0, self.class_labels: test_labels})
	...
	def train(self):
		...
		with tf.Session(config=config, graph=self.graph) as sess:
			...
			while epoch < self.model_epochs:
				...
				while train_batch_number < self.num_train_batches:
					...
					corrupted_train_images = preprocess_input(train_images, shift=self.shift_input) #, noise=self.noise, cutout=self.cutout)
		...
		avg_test_accuracy, avg_test_confidence = self.check_test_accuracy(sess, test_set, deep=True,
																		  show_preds=False)
		total_test_loss, dcl_loss, avg_test_ae_loss = self.check_test_loss(sess, self.num_test_batches,
																		   test_set=test_set)

		print(
		"Epoch {:d}/{:d}: {} | Loss: train {:.5f}, test {:.5f} (cl {:.3f} ae {:.3f}) | Accuracy: train {:.3f} test {:.3f} (conf {:.3f})".format(
			epoch + 1, self.model_epochs, str(datetime.now())[11:-7], avg_train_loss, total_test_loss, dcl_loss,
			self.ae_weight * avg_test_ae_loss, avg_train_accuracy, avg_test_accuracy, avg_test_confidence))
